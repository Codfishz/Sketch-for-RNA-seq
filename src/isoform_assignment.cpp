...
std::unordered_map<std::string, int> assign_reads_to_isoforms(
    const std::unordered_map<std::string, std::vector<std::string>>& homologous_segments,
    const std::unordered_map<std::string, Transcript>& transcripts) {

    std::unordered_map<std::string, int> read_counts;

    for (const auto& [read_id, transcript_ids] : homologous_segments) {
        for (const auto& transcript_id : transcript_ids) {
